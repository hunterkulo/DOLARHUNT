...
const isOAuthUser = () => {
    ...
    return authType === 'oauth' && !!clientAccounts;
};
...
const AppContent = observer(() => {
    ...
    const changeActiveSymbolLoadingState = () => {
        ...
        // For OAuth users, skip the active symbols loading
        if (isOAuthUser()) {
            console.log('[AppContent] OAuth user detected, skipping active symbols loading');
            setIsLoading(false);
            return;
        }
    ...
    React.useEffect(() => {
        if (is_api_initialized) {
            ...
            // For OAuth users, we can skip loading and show app immediately
            if (isOAuthUser()) {
                console.log('[AppContent] OAuth user, setting loading to false immediately');
                setIsLoading(false);
                return;
            }
    ...
    React.useEffect(() => {
        if (client.is_logged_in && client.is_landing_company_loaded && is_api_initialized) {
            // For OAuth users, we already set loading to false
            if (!isOAuthUser()) {
                changeActiveSymbolLoadingState();
            }
        }
    ...
    // Show loading for OAuth users? No - show app immediately
    if (is_loading && !isOAuthUser()) {
        return <ChunkLoader message={localize('Initializing your account...')} />;
    }

    // For OAuth users or when loading is complete, show the app
    return (
        <>
